        ...
        <div class="admin-card-body" style="display:flex;flex-direction:column;gap:var(--sp-5)">
            ...
            <div class="aform-group">
                ...
                <div style="display:flex;gap:var(--sp-2)">
                    <input type="text" name="code" id="couponCode"
                        ...
                        value="{{ old('code', isset($coupon) ? $coupon->code : '') }}" placeholder="e.g. SAVE15"
                        style="text-transform:uppercase;font-family:var(--font-mono);font-weight:var(--weight-semibold);letter-spacing:0.05em">
                    @if(!isset($coupon))
                    <button type="button" onclick="generateCode()" class="abtn abtn-outline abtn-sm"
            ...
            <div class="layout-grid-main-aside">
                <div class="aform-group">
                    ...
                    <select name="type" id="couponType" class="aform-control @error('type') is-invalid @enderror"
                        ...
                        <option value="fixed" {{ old('type', $coupon->type ?? '') === 'fixed' ? 'selected':'' }}>Fixed
                            Amount ($)</option>
                        <option value="free_shipping" {{ old('type', $coupon->type ?? '') === 'free_shipping' ?
                            'selected':'' }}>Free Shipping</option>
                    </select>
                ...
                <div class="aform-group" id="valueGroup">
                    ...
                    <div style="position:relative">
                        <span id="valuePrefix"
                            style="position:absolute;left:12px;top:50%;transform:translateY(-50%);color:var(--admin-muted);font-size:var(--text-sm)"></span>
                        <input type="number" name="value" id="couponValue"
                            class="aform-control @error('value') is-invalid @enderror"
                            value="{{ old('value', $coupon->value ?? '') }}" min="0" step="0.01"
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--sp-4)">
                <div class="aform-group">
                    ...
                    <div style="position:relative">
                        <span
                            style="position:absolute;left:12px;top:50%;transform:translateY(-50%);color:var(--admin-muted);font-size:var(--text-sm)">{{
                            $currencySymbol }}</span>
            ...
            <div class="aform-toggle-row">
                <label class="aform-toggle">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $coupon->is_active ?? true) ?
                    'checked' : '' }}>
